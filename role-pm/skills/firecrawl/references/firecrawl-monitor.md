...
- [firecrawl-scrape](firecrawl-scrape.md) — one-off scrape; escalate to `monitor` when checks become recurring
- [firecrawl-crawl](firecrawl-crawl.md) — one-off crawl; pair with `--crawl-url` here for recurring crawl diffs
- [firecrawl-cli](../SKILL.md) — top-level workflow guide
