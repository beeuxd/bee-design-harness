...
**HARD GATE (all builders):** if `docs/design-system.md` has no `## Art direction` section, visual build skills (`bootstrap-app`, `from-figma`, `new-site-component`, `design-flow`, `prd-to-ui`, `design-options`, `motion`) must STOP and run this skill first. Every visual build must cite which reference each section follows.
...
- **`grep: docs/design-system.md: No such file or directory`** → the project's docs aren't in place. STOP and ask the user whether the project has been bootstrapped (`bootstrap-app` creates the doc set) — do not create `docs/design-system.md` yourself from this skill.
...
Ask all seven questions in a single message. Then STOP and wait for the user's reply. Never answer for the user. Never propose "defaults" as if accepted. If the user skips an item, record it as `OPEN — [question]` in the doc.
...
6. **Motion personality** — on a scale from static → expressive; this feeds the `motion` skill.
...
### Motion personality
[static → expressive placement; notes for the motion skill]
...
- Pass the named references (with their "what to take" notes) to the `design-references` skill to capture screenshots + extracted notes into `design/references/`.
...
- The user defers every question — do not fabricate a direction; propose running `design-references` on 2–3 candidate directions instead and wait.
