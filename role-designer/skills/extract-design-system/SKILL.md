...
7. In this harness the token source of truth is `design/tokens.json` (a protected file — edits always require user confirmation). Treat the generated `design-system/` files as starter artifacts only: propose the mapping into `design/tokens.json`, get explicit approval, then run `pnpm tokens` to regenerate `app/tokens.css`. Never hand-edit generated CSS and never overwrite `design/tokens.json` directly with extracted output.
